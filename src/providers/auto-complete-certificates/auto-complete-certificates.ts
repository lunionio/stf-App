...
@Injectable()
export class AutoCompleteCertificatesProvider implements AutoCompleteService {

  private keywords: Array<string>;
  public blankResult: string; 

  labelAttribute = "certificateName";
  ...
  getResults(keyword: string) {
    if (this.keywords.length) {
      ...
      return this.listingsProvider.getCertificates().subscribe((result) => {
        this.keywords = result.map(item => item.certificateName);
        return this.filter(keyword);
      }, (err) => {
        return [];
      });
